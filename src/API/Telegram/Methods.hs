...
sendMessage :: (Monad m) => Handle m -> T.SendMessage -> m Request
sendMessage h msg = 
   return
...
-- UNTESTED
sendSticker :: (Monad m) => Handle m -> T.SendSticker -> m Request
...
getFile :: (Monad m)
        => Handle m  -- ^ Telegram methods constructor @Handle@
        -> Text      -- ^ file_id
        -> m Request   -- ^ GET Request with empty body and query
getFile h fileId =
   return
...
fileTelegramRequest h filePath = let
   ...
   in return
   $ setRequestHost (cfgHost . hConfig $ h)
   $ setRequestPath path
   $ defaultRequest
